...
//    NSLog(@"这个文件已经存在：%@",result?@"是的":@"不存在");
    ...
    }else if ([commandIdentifier isEqual:@"clangTest"])
    {
        ...
        [self clangAST:pyPath :pyScriptPath];
    }
    
    completionHandler(nil);
}
-(void)clangAST:(NSString*) clangPath:(NSString*) pyScriptPath
{
    NSString *cmd = [NSString stringWithFormat:@"clang -fmodules -fsyntax-only -Xclang -ast-dump %@",pyScriptPath];
